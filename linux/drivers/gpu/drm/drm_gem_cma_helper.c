 ...
 */
static struct drm_gem_cma_object *__drm_gem_cma_create(struct drm_device *drm,
						       size_t size)
{
 ...
 * 0 on success or a negative error code on failure.
 */
int drm_gem_cma_dumb_create(struct drm_file *file_priv, struct drm_device *drm,
			    struct drm_mode_create_dumb *args)
{
	struct drm_gem_cma_object *cma_obj;
...
int drm_gem_cma_prime_vmap(struct drm_gem_object *obj, struct iosys_map *map)
{
				       ...
				       struct sg_table *sgt)
{
	...
	// vaddr = dma_buf_vmap(attach->dmabuf, &map);
